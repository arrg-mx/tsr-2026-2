...
from jetson_utils.jtop_utils import (
    other_status,
    board_status,
    disk_status,
    cpu_status,
...
class JtopUtils:
    ...
    def __init__(self, level_options):
        ...
        interval = 0.5 #rospy.get_param("~interval", 0.5)
        # Initialization jtop
    ...
    def start(self):
        ...
        if self.jetson.interval != self.jetson.interval_user:
            ...
            print("I cannot set {user:.2f}s. Service already running at {service:.2f}s".format(user=self.jetson.interval_user, service=self.jetson.interval))
    ...
    def jetson_callback(self, jetson):
        ...
        # Update status jtop
        # rospy.logidebug("jtop message %s" % rospy.get_time())
        curr_time = datetime.now()
        print("jtop message %s" % current_time.strftime("%H:%M:%S"))
...
def wrapper():
    ...
    jetson = JtopUtils(level_options)
